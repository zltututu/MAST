    ...
    def after_fit(self):
        if not self.every_epoch and self.global_rank == self.save_process_id:
            if self.last_saved_path is not None:
                ...
                # If no model was saved (e.g. valid_loss was NaN), try loading the last epoch's model.
                import os
                model_path = os.path.join(self.path, f'{self.fname}.pth')
                if os.path.exists(model_path):
                    self.learner.load(model_path, with_opt=self.with_opt)
